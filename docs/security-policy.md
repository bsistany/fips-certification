...
### ACVP-Format Algorithm Testing (Sprint 8)

In addition to the runtime KATs above, all four approved algorithms have been
validated against an independent reference implementation using ACVP-format
JSON vectors. This is distinct from the power-on self-tests — it is a
development-time validation activity, not a runtime check.

| Algorithm | ACVP Algorithm String | Vectors | Result |
|---|---|---|---|
| AES-256-CBC | ACVP-AES-CBC | 8 (4 encrypt, 4 decrypt) | ✅ 8/8 passed |
| SHA-256 | ACVP-SHA2-256 | 11 (AFT: empty, short, boundary) | ✅ 11/11 passed |
...
In addition, all four algorithms were validated using ACVP-format vectors
generated by an independent Python reference (Sprint 8). See B.2.10 and
`docs/acvp_validation_prompt.md` for details. Total: 54 unit tests + 43
ACVP vectors, all passing.
...
|---|---|---|---|---|
| AES | FIPS 197, SP 800-38A | 128-bit, 256-bit | CBC | ✅ ACVP local simulation complete — Sprint 8 |
| SHA-256 | FIPS 180-4 | 256-bit output | — | ✅ ACVP local simulation complete — Sprint 8 |
| HMAC-SHA-256 | FIPS 198-1 | Key ≥ 112 bits | — | ✅ ACVP local simulation complete — Sprint 8 |
| PBKDF2-HMAC-SHA-256 | SP 800-132 | Variable | Iterations ≥ 1000, Salt ≥ 16 bytes | ✅ ACVP local simulation complete — Sprint 8 |
...
| 8 | No CAVP certificates | Appendix A | ACVP local simulation complete (Sprint 8); demo server submission pending (Sprint 9) | CMVP prerequisite |
